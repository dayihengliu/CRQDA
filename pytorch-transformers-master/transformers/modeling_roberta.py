...
class RobertaForQuestionAnsweringClassify(BertPreTrainedModel):
    r"""
        **start_positions**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for position (index) of the start of the labelled span for computing the token classification loss.
            Positions are clamped to the length of the sequence (`sequence_length`).
    ...
    base_model_prefix = "roberta"
    def __init__(self, config):
        super(RobertaForQuestionAnsweringClassify, self).__init__(config)
        print('cg roberta===========================')
        print('')
        self.roberta = RobertaModel(config)
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
        ...
        if labels is not None:
            if self.num_labels == 1:
                #  We are doing regression
                c_loss_fct = BCEWithLogitsLoss()
                c_loss = c_loss_fct(classification_logits, labels)
            else:
...
class RobertaForQuestionAnsweringClassifyCLS(BertPreTrainedModel):
    r"""
        **start_positions**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for position (index) of the start of the labelled span for computing the token classification loss.
            Positions are clamped to the length of the sequence (`sequence_length`).
    ...
    base_model_prefix = "roberta"
    def __init__(self, config):
        super(RobertaForQuestionAnsweringClassifyCLS, self).__init__(config)
        print('cg roberta===========================')
        #print('')
        self.roberta = RobertaModel(config)
        #print('self.roberta.embeddings', self.roberta.embeddings)
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
        ...
        if labels is not None:
            if self.num_labels == 1:
                ...
                c_loss = c_loss_fct(classification_logits.view(-1), labels.to(classification_logits))
            else:
                c_loss_fct = CrossEntropyLoss()
                c_loss = c_loss_fct(classification_logits.view(-1, self.num_labels), labels.view(-1))
        ...
        return outputs  # (loss), start_logits, end_logits, (hidden_states), (attentions), cls
