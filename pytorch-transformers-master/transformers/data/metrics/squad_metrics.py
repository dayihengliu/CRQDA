...
def compute_predictions_logits_cls_add(
    all_examples,
    all_features,
    all_results,
    n_best_size,
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        for (feature_index, feature) in enumerate(features):
            ...
            if version_2_with_negative:
                feature_null_score = result.cls_logits[0] + result.start_logits[0] + result.end_logits[0]
                if feature_null_score < score_null:
                    score_null = feature_null_score
                    min_null_feature_index = feature_index
                    null_start_logit = result.cls_logits[0] + result.start_logits[0]
...
def compute_predictions_logits_cls(
    all_examples,
    all_features,
    all_results,
    n_best_size,
        ...
        for (feature_index, feature) in enumerate(features):
            ...
            if version_2_with_negative:
                ...
                if feature_null_score < score_null:
                    ...
                    min_null_feature_index = feature_index
                    null_start_logit = result.cls_logits[0] #result.start_logits[0]
                    null_end_logit = 0#result.end_logits[0]
            for start_index in start_indexes:
                for end_index in end_indexes:
                    # We could hypothetically create invalid predictions, e.g., predict
                    # that the start of the span is in the question. We throw out all
