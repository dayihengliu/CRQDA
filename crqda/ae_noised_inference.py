...
def test_mrc(args, model, tokenizer, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            #print('classification_logits', classification_logits.shape, classification_logits)
            #(loss), start_logits, end_logits, (hidden_states), (attentions), classification_logits
...
"""
save_model = torch.load(mata_args.mrc_model_path)
...
output_dir = 'gen_res_noised/' + model_prefix
...
for batch in tqdm(eval_dataloader, desc="Evaluating"):
    ...
    for epsilon in epsilon_list:
        #inputs = pickle.load(open('sample_inputs.pkl', 'rb'))
        if args.n_gpu > 1:
        ...
        for _ in range(5):
            ...
            #print(c_loss.data.item())
            #print(classification_logits.cpu().detach().numpy())
            #print('closs %.2f' % c_loss.data.item(), 'logits', classification_logits[0])
            ...
            #print('mrc_masks_expand', mrc_masks_expand.shape)
            #print('embedding_mrc_grad', embedding_mrc_grad.shape)
            #print('embedding_mrc_grad * mrc_masks_expand', embedding_mrc_grad * mrc_masks_expand)
            #prev_mrc = embedding_mrc.clone()
            #prev_dag = embedding_dag.clone()
            #=====
            embedding_dag_grad = embedding_mrc_grad[:, :max_query_length, :] * question_masks_expand
            t_mean = torch.unsqueeze(embedding_dag_grad.mean(dim=-1), -1).repeat(1,1,embedding_dag_grad.size()[-1])
            t_std = torch.unsqueeze(embedding_dag_grad.std(dim=-1), -1).repeat(1,1,embedding_dag_grad.size()[-1])
            embedding_dag_grad_noise = torch.normal(mean=t_mean, std=t_std)
            
            t_mean = torch.unsqueeze(embedding_mrc_grad.mean(dim=-1), -1).repeat(1,1,embedding_mrc_grad.size()[-1])
            t_std = torch.unsqueeze(embedding_mrc_grad.std(dim=-1), -1).repeat(1,1,embedding_mrc_grad.size()[-1])
            embedding_mrc_grad_noise = torch.normal(mean=t_mean, std=t_std)
            #====
            embedding_dag = embedding_dag - epsilon * embedding_dag_grad_noise * scaled
            embedding_mrc = embedding_mrc - epsilon * embedding_mrc_grad_noise * mrc_masks_expand * scaled
            #print('prev_mrc - embedding_mrc', (prev_mrc - embedding_mrc).mean().data.item())
            #print('prev_dag - embedding_dag', (prev_dag - embedding_dag).mean().data.item())
            #print('embedding_mrc_grad', embedding_mrc_grad.sum().data.item()*10*epsilon)
            #print('embedding_mrc_grad_mask', epsilon * (embedding_mrc_grad * mrc_masks_expand).sum().data.item() * 10.)
            #print('embedding_dag', embedding_dag.sum().data.item())
            #print('c_epsilon', epsilon)
            ...
            #    print('prev_latent-latent', (prev_latent-latent).mean().item())
                ...
                if uniq_id + i not in gen_results:
                    ...
                    if jacard_dist < JACARD_UPPER and jacard_dist > JACARD_LOWER:
                        ...
                        if len(gen_results[uniq_id + i]['gen']) > 0 and gen_sample != gen_results[uniq_id + i]['gen'][-1][0]:
                            ...
                            gen_results[uniq_id + i]['gen'].append((gen_sample, jacard_dist, original_epsilon, _, score))
            #print(_)
            #print(postprocess_raw(q[0]), postprocess_gen(g[0]), Jaccrad(postprocess_raw(q[0]), postprocess_gen(g[0])), '\n')
            #print(postprocess_raw(q[1]), postprocess_gen(g[1]), Jaccrad(postprocess_raw(q[1]), postprocess_gen(g[1])),'\n')
            #print(postprocess_raw(q[2]), postprocess_gen(g[2]), Jaccrad(postprocess_raw(q[2]), postprocess_gen(g[2])),'\n')
            #print(postprocess_raw(q[3]), postprocess_gen(g[3]), Jaccrad(postprocess_raw(q[3]), postprocess_gen(g[3])),'\n')
            #print(postprocess_raw(q[4]), postprocess_gen(g[4]), Jaccrad(postprocess_raw(q[4]), postprocess_gen(g[4])),'\n')
            #print(postprocess_raw(q[5]), postprocess_gen(g[5]), Jaccrad(postprocess_raw(q[5]), postprocess_gen(g[5])),'\n\n')

            epsilon = epsilon * 0.9
            #print(postprocess_doc(d[0]))
    #break
